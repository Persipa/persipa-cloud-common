...
运行环境要求 JDK 17 或更高版本。当前仓库默认构建版本为 `4.3.0`。调用方应以 Maven
最终解析版本和仓库中的实际制品为准；根 POM 的版本号不表示正式版本已发布。
...
以下按仓库提交归纳功能演进。正式发布状态以 Git Tag 和 Maven 仓库为准；调用方不能仅凭本仓库
默认版本或开发分支推断制品已发布。

| 版本 | 主要变化 |
| --- | --- |
| `4.3.0` | 新增默认关闭的 Servlet MVC CORS 自动配置，可设置来源、路径、方法、请求头、凭证和缓存时长；`4.2.2` 不包含该能力。 |
| `4.2.2` | 完善 MyBatis-Plus `Instant` 时间字段自动填充的接入说明；此版本的提交未新增运行时功能。 |
| `4.2.1` | 包含 4.2 开发阶段的 core/starter 模块拆分、分页响应与转换、分页和乐观锁拦截器，以及应用版本 HTTP 接口。旧聚合坐标不再提供 Java 类。 |
| `4.1.x` | 引入可配置的 `Instant` 时间字段填充，并逐步完善应用 Ready 后的版本信息打印。 |
| `4.0.x` | 提供统一响应模型，以及 Jackson、MyBatis-Plus 和 Springdoc 的按需自动配置。 |

## 已实现功能

当前 `4.3.0` 源码包含以下能力；各能力默认关闭或按调用方显式使用，具体启用条件见下文。

| 模块 | 能力 |
| --- | --- |
| core | `Result<T>`、`PageResponse<T>` 和 `EnumFindHelper`；无 Spring 或其他框架运行时依赖。 |
| starter | Jackson `LocalDateTime` 格式配置；MyBatis-Plus 时间字段填充、分页、乐观锁和分页结果转换；Springdoc `OpenAPI` 默认 Bean。 |
| starter | Servlet MVC CORS 配置；应用 Ready 后打印构建信息；可配置的应用版本 HTTP 接口。 |

starter 传递依赖 core。MyBatis-Plus 和 Springdoc 等可选集成需要调用方显式引入匹配的依赖；
自动配置只在所需类存在且配置显式启用时生效，并在调用方提供对应 Bean 时退让。
...
仅在业务方需要浏览器跨域访问、且实际解析的 starter 版本包含该能力时启用；
正式版本从 `4.3.0` 引入，使用 `4.3.0-SNAPSHOT` 时还需核对其实际构建内容。
